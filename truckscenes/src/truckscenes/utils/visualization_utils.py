...
class TruckScenesExplorer:
    ...
    def render_pointcloud_in_image(self,
        ...
        points, coloring, im, mask = self.map_pointcloud_to_image(pointsensor_token, camera_token,
        ...
        return points, coloring, im, ax, mask
    
    def render_pointcloud_in_image_subplot(self,
                                   sample_token: str,
                                   dot_size: int = 4,
                                   pointsensor_channel: str = 'LIDAR_LEFT',
                                   camera_channel: str = 'CAMERA_LEFT_FRONT',
        ...
        # Init axes.
        if ax1 is None:
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(9, 16))
            ax1.set_title(sample_token)
            ax2.set_title(sample_token)
        else:  # Set title on if rendering as part of render_sample.
            ax1.set_title(camera_channel)
            ax2.set_title(camera_channel)
        # ax1.imshow(im)
        # ax1.scatter(points[0, :], points[1, :], marker='o', c=coloring,
        #            s=dot_size, edgecolors='none')
        # ax1.axis('off')
        # ax2.imshow(im)
        # ax2.scatter(points[0, :], points[1, :], marker='o', c=coloring,
        #            s=dot_size, edgecolors='none')
        # ax2.axis('off')
        # plt.tight_layout()
        ...
        return points, coloring, im, ax1, ax2, mask
    ...
    def render_sample_lidar_only(self,
                      token: str,
                      box_vis_level: BoxVisibility = BoxVisibility.ANY,
                      nsweeps: int = 1,
                      out_path: str = None,
                      verbose: bool = True) -> None:
        """
        Render and display only the fused LIDAR sample data (no radar or camera).

        Arguments:
            token: Sample token.
            nsweeps: Number of LIDAR sweeps to fuse.
        """
        

        record = self.trucksc.get('sample', token)

        # Extract LIDAR data channels
        lidar_data = {
            channel: token
            for channel, token in record['data'].items()
            if self.trucksc.get('sample_data', token)['sensor_modality'] == 'lidar'
        }

        if not lidar_data:
            print("No LIDAR data available for this sample.")
            return

        # Create a single plot for fused LIDAR
        fig, ax = plt.subplots(figsize=(12, 8))
        for i, (_, sd_token) in enumerate(lidar_data.items()):
            # Only show annotations for the first one
            self.render_sample_data(sd_token,
                                    with_anns=(i == 0),
        ...
        ax.set_title('Fused LIDAR View')
        plt.tight_layout()
        plt.show()

    def render_sample_radar_only(self,
                      token: str,
                      box_vis_level: BoxVisibility = BoxVisibility.ANY,
                      nsweeps: int = 1,
                      out_path: str = None,
                      verbose: bool = True) -> None:
        """
        Render and display only the fused RADAR sample data (no LIDAR or camera).

        Arguments:
            token: Sample token.
            nsweeps: Number of RADAR sweeps to fuse.
        """
        import matplotlib.pyplot as plt
        import numpy as np
        ...
        radar_data = {
            channel: token
            for channel, token in record['data'].items()
            if self.trucksc.get('sample_data', token)['sensor_modality'] == 'radar'
        }
    ...
    def render_sample_radar_lidar(self,
                      token: str,
                      box_vis_level: BoxVisibility = BoxVisibility.ANY,
                      nsweeps: int = 1,
                      out_path: str = None,
                      verbose: bool = True) -> None:
        """
        Render and display the fused LIDAR and RADAR sample data side by side.

        Arguments:
            token: Sample token.
            box_vis_level: Visibility level for annotation boxes.
            nsweeps: Number of sweeps to fuse.
        """
        import matplotlib.pyplot as plt
        import numpy as np
        ...
        # Separate LIDAR and RADAR data
        for channel, sd_token in record['data'].items():
            sd_record = self.trucksc.get('sample_data', sd_token)
            modality = sd_record['sensor_modality']
            if modality == 'lidar':
                lidar_data[channel] = sd_token
            elif modality == 'radar':
                radar_data[channel] = sd_token

        # Check if there's any data to render
        if not lidar_data and not radar_data:
            print("No LIDAR or RADAR data available for this sample.")
            return

        # Create a 1-row, 2-column plot (or 1 if only one type is available)
        ncols = int(bool(lidar_data)) + int(bool(radar_data))
        fig, axes = plt.subplots(1, ncols, figsize=(16, 8))
        if ncols == 1:
    ...
    def render_sample_lidar_with_safe_space(self,
                              token: str,
        ...
        """
        Render and display only the fused LIDAR sample data (no radar or camera),
        with an overlay of the estimated free area using a 2D occupancy grid.
        ...
        lidar_data = {
            channel: tok
            for channel, tok in record['data'].items()
            if self.trucksc.get('sample_data', tok)['sensor_modality'] == 'lidar'
        }
        ...
        # ===== Add free area overlay using occupancy grid =====
        first_channel = next(iter(lidar_data))
        sd_token = lidar_data[first_channel]
        sd_record = self.trucksc.get('sample_data', sd_token)
        sample = self.trucksc.get('sample', token)
        chan = sd_record['channel']
        ref_chan = 'LIDAR_LEFT'

        try:
            pc, _ = LidarPointCloud.from_file_multisweep(self.trucksc, sample, chan, ref_chan, nsweeps=nsweeps)
            points = pc.points[:3, :].T  # Shape: (N, 3)

            # Step 1: Filter near-ground points
            ground_points = points[np.abs(points[:, 2]) < 0.3][:, :2]
            ...
            x_bins = int((x_limits[1] - x_limits[0]) / resolution)
            y_bins = int((y_limits[1] - y_limits[0]) / resolution)

            occupancy_grid = np.zeros((x_bins, y_bins), dtype=np.uint8)

            x_idx = ((ground_points[:, 0] - x_limits[0]) / resolution).astype(int)
            y_idx = ((ground_points[:, 1] - y_limits[0]) / resolution).astype(int)

            in_bounds = (x_idx >= 0) & (x_idx < x_bins) & (y_idx >= 0) & (y_idx < y_bins)
            x_idx = x_idx[in_bounds]
            y_idx = y_idx[in_bounds]

            occupancy_grid[x_idx, y_idx] = 1

            # Step 3: Morphological closing to fill free space
            import cv2
            inv_grid = (occupancy_grid == 0).astype(np.uint8) * 255
            kernel = np.ones((5, 5), np.uint8)
            free_space_mask = cv2.morphologyEx(inv_grid, cv2.MORPH_CLOSE, kernel)

            # Step 4: Compute free area
            free_cells = np.sum(free_space_mask == 255)
            free_area_m2 = free_cells * (resolution ** 2)

            # Step 5: Overlay on plot
            free_x_idx, free_y_idx = np.where(free_space_mask == 255)
            free_x = free_x_idx * resolution + x_limits[0]
            free_y = free_y_idx * resolution + y_limits[0]
            ax.scatter(free_x, free_y, s=1, c='red', label='Free space (grid)')

            ax.legend()
            print(f"Estimated Free Space Area: {free_area_m2:.2f} m²")

        except Exception as e:
            print(f"Error computing free space area: {e}")

        ax.set_title('Fused LIDAR View with Free Space Overlay')
        plt.tight_layout()
        if out_path:
            plt.savefig(out_path)
            if verbose:
    ...
    def get_pointcloud(self,
        ...
        """
        Returns concatenated 3D point cloud for the given channels.
        
        Arguments:
            sample_rec: Sample record.
            chans: Sensor channels to retrieve.
            ref_chan: Reference sensor channel.
            nsweeps: Number of sweeps for lidar/radar.
            min_distance: Minimum distance to include points.
        
        Returns:
            point_clouds: (N, 3) array of 3D points (x, y, z) in ref_chan frame.
        """
        if not isinstance(chans, list):
            chans = [chans]
